...
@section('page-title', 'Editar Campo Extra')

@section('content')
<div class="space-y-6">
    <form method="POST" action="{{ route('admin.extra-fields.update', $extraField) }}">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" 
                           id="name" 
                           name="name" 
                           value="{{ old('name', $extraField->name) }}"
                ...
                <div>
                    ...
                    <select id="type" 
                        ...
                        <option value="text" {{ old('type', $extraField->type) == 'text' ? 'selected' : '' }}>Texto</option>
                        <option value="textarea" {{ old('type', $extraField->type) == 'textarea' ? 'selected' : '' }}>Área de Texto</option>
                        <option value="select" {{ old('type', $extraField->type) == 'select' ? 'selected' : '' }}>Seleção (Dropdown)</option>
                        <option value="radio" {{ old('type', $extraField->type) == 'radio' ? 'selected' : '' }}>Radio Buttons</option>
                        <option value="checkbox" {{ old('type', $extraField->type) == 'checkbox' ? 'selected' : '' }}>Checkboxes</option>
                        <option value="image" {{ old('type', $extraField->type) == 'image' ? 'selected' : '' }}>Seleção Visual (Imagens)</option>
                        <option value="color" {{ old('type', $extraField->type) == 'color' ? 'selected' : '' }}>Seleção Visual (Cores)</option>
                        <option value="number" {{ old('type', $extraField->type) == 'number' ? 'selected' : '' }}>Número</option>
                        <option value="date" {{ old('type', $extraField->type) == 'date' ? 'selected' : '' }}>Data</option>
                        <option value="file" {{ old('type', $extraField->type) == 'file' ? 'selected' : '' }}>Arquivo</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6">
                ...
                <textarea id="description" 
                          name="description" 
                          rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('description') border-red-500 @enderror">{{ old('description', $extraField->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="flex items-center">
                    <input type="checkbox" 
                           ...
                           name="is_required" 
                           value="1"
                           {{ old('is_required', $extraField->is_required) ? 'checked' : '' }}
                ...
                <div class="flex items-center">
                    <input type="checkbox" 
                           ...
                           name="is_active" 
                           value="1"
                           {{ old('is_active', $extraField->is_active) ? 'checked' : '' }}
                           class="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('settings.placeholder', $extraField->settings['placeholder'] ?? '') }}"
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   id="settings_unit" 
                                   name="settings[unit]" 
                                   value="{{ old('settings.unit', $extraField->settings['unit'] ?? '') }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" 
                                   id="settings_min" 
                                   name="settings[min]" 
                                   value="{{ old('settings.min', $extraField->settings['min'] ?? '') }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                        </div>
                        <div>
                            ...
                            <input type="number" 
                                   id="settings_max" 
                                   name="settings[max]" 
                                   value="{{ old('settings.max', $extraField->settings['max'] ?? '') }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                        </div>
                        <div>
                            ...
                            <input type="number" 
                                   id="settings_step" 
                                   name="settings[step]" 
                                   value="{{ old('settings.step', $extraField->settings['step'] ?? 1) }}"
                                   step="0.01"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" 
                                   id="settings_rows" 
                                   name="settings[rows]" 
                                   value="{{ old('settings.rows', $extraField->settings['rows'] ?? 3) }}"
                                   min="1"
                                   max="20"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" 
                               ...
                               value="{{ old('validation_rules.min_length', $extraField->validation_rules['min_length'] ?? '') }}"
                               min="0"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                    </div>
                    <div>
                        ...
                        <input type="number" 
                               ...
                               value="{{ old('validation_rules.max_length', $extraField->validation_rules['max_length'] ?? '') }}"
                               min="0"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
                    </div>
                </div>
            ...
            <div class="mt-6">
                ...
                <input type="number" 
                       id="sort_order" 
                       name="sort_order" 
                       value="{{ old('sort_order', $extraField->sort_order) }}"
                       min="0"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
