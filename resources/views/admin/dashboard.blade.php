...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6 relative overflow-hidden">
            ...
            @endphp
            <div class="absolute left-0 top-0 bottom-0 w-1 {{ $barColor }}" style="height: {{ min($growthPercent, 100) }}%;"></div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-4 flex-1">
                    ...
                    <div class="flex items-center mt-1">
                        <i class="bi bi-{{ $isPositive ? 'arrow-up' : 'arrow-down' }} text-xs {{ $isPositive ? 'text-green-600' : 'text-red-600' }} mr-1"></i>
                        <span class="text-xs font-medium {{ $isPositive ? 'text-green-600' : 'text-red-600' }}">
                            {{ number_format($growthPercent, 1) }}% vs mês anterior
                        </span>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 relative overflow-hidden">
        ...
        @endphp
        <div class="absolute left-0 top-0 bottom-0 w-1.5 {{ $revenueBarColor }}" style="height: {{ min($revenueGrowthPercent, 100) }}%;"></div>
        
        <div class="flex items-center justify-between">
            <div class="flex-1">
                <h3 class="text-lg font-medium text-gray-900">Receita Estimada</h3>
                <p class="text-3xl font-bold text-primary">R$ {{ number_format($stats['total_revenue'] ?? 0, 2, ',', '.') }}</p>
                <div class="flex items-center mt-2">
                    <i class="bi bi-{{ $isRevenuePositive ? 'arrow-up' : 'arrow-down' }} text-sm {{ $isRevenuePositive ? 'text-green-600' : 'text-red-600' }} mr-1"></i>
                    <span class="text-sm font-medium {{ $isRevenuePositive ? 'text-green-600' : 'text-red-600' }}">
                        {{ number_format($revenueGrowthPercent, 1) }}% vs mês anterior
                    </span>
                    <span class="text-xs text-gray-500 ml-2">
                        (R$ {{ number_format($stats['current_month_revenue'] ?? 0, 2, ',', '.') }} este mês)
                    </span>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <a href="{{ route('admin.products.create') }}" class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
            <div class="flex items-center">
                <div class="selo rounded-full bg-blue-100 text-blue-600">
        ...
        <a href="{{ route('admin.categories.create') }}" class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
            <div class="flex items-center">
                <div class="selo rounded-full bg-green-100 text-green-600">
        ...
        <a href="{{ route('admin.orders') }}" class="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
            <div class="flex items-center">
                <div class="selo rounded-full bg-yellow-100 text-yellow-600">
                    <i class="bi bi-cart-check text-2xl"></i>
                </div>
