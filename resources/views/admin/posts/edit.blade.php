...
@section('title', 'Editar Post - Laser Link')
@section('page-title', 'Editar Post')

@push('head')
<!-- Desabilitar cache para página de edição de posts -->
        ...
        <div class="flex gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="excerpt" 
                                      name="excerpt" 
                                      rows="3"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('excerpt') border-red-500 @enderror">{{ old('excerpt', $post->excerpt) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea id="content" 
                                      name="content" 
                                      rows="20"
                                      class="quill-editor">{{ old('content', $post->content) }}</textarea>
                            @error('content')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="meta_description" 
                                      ...
                                      rows="2"
                                      maxlength="160"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('meta_description') border-red-500 @enderror">{{ old('meta_description', $post->meta_description) }}</textarea>
                        ...
                        <div x-data="keywordsTags()">
                            ...
                            <!-- Tags Display -->
                            <div class="mb-2 min-h-[42px] p-2 border border-gray-300 rounded-lg bg-gray-50 flex flex-wrap gap-2 items-center">
                                <template x-for="(tag, index) in tags" :key="index">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-gray-50 bg-opacity-10 text-black hover:bg-gray-100 border border-gray-500 shadow-sm hover:bg-transparent transition-colors">
                                ...
                                <input type="text" 
                                       ...
                                       class="flex-1 min-w-[200px] text-sm bg-transparent border-none focus:outline-none">
                            </div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                ...
                                Digite uma palavra-chave e pressione Enter ou vírgula para adicionar
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="w-80 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                            ...
                            <span class="text-lg font-semibold text-primary">{{ number_format($post->views) }}</span>
                        </div>
                        <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                            <span class="text-sm text-gray-600">Tempo de leitura:</span>
                            <span class="text-sm font-semibold">~{{ $post->reading_time }} min</span>
                        </div>
                        <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                            <span class="text-sm text-gray-600">Criado em:</span>
                            <span class="text-sm font-semibold">{{ $post->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600">Atualizado em:</span>
                            <span class="text-sm font-semibold">{{ $post->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <p class="text-xs text-gray-500 mt-2">Use a IA para gerar conteúdo completo baseado no título</p>
                </div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <div class="flex flex-col gap-3">
                        @if ($post->isPublished())
                            <a href="{{ $post->url }}" 
                               ...
                               class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-center">
                                <i class="bi bi-box-arrow-up-right mr-2"></i>Ver Post
                            </a>
                        @endif
                        
                        <button type="submit" 
                                class="w-full bg-primary text-white px-4 py-2 rounded-lg hover:opacity-90 transition-colors">
                            <i class="bi bi-check-lg mr-2"></i>Atualizar Post
                        </button>
                        ...
                        <button type="button" 
                                onclick="if(confirm('Tem certeza que deseja deletar este post?')) document.getElementById('delete-form').submit()"
                                class="w-full bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors">
                            <i class="bi bi-trash mr-2"></i>Deletar Post
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    ...
    window.postFeaturedImage = '{{ $post->featured_image ? asset("storage/" . $post->featured_image) : "" }}';
