...
@section('content')
<div class="space-y-6">
    <form action="{{ route('admin.raw-materials.update', $rawMaterial) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('description') border-red-500 @enderror">{{ old('description', $rawMaterial->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="specifications" id="specifications" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('specifications') border-red-500 @enderror">{{ old('specifications', $rawMaterial->specifications) }}</textarea>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="unit_cost" id="unit_cost" value="{{ old('unit_cost', $rawMaterial->unit_cost) }}" 
            ...
            <div class="p-4 bg-blue-50 rounded-lg mt-6">
                <p class="text-sm text-gray-700">
                    <strong>Estoque Atual:</strong> {{ number_format($rawMaterial->stock_quantity, 3, ',', '.') }} {{ $rawMaterial->unit_label }}
                </p>
                <p class="text-xs text-gray-500 mt-1">Use a página de movimentações para alterar o estoque</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    <label for="stock_min" class="block text-sm font-medium text-gray-700 mb-2">Estoque Mínimo *</label>
                    <input type="number" name="stock_min" id="stock_min" value="{{ old('stock_min', $rawMaterial->stock_min) }}" 
                ...
                <div>
                    ...
                    <input type="number" name="stock_max" id="stock_max" value="{{ old('stock_max', $rawMaterial->stock_max) }}" 
                           step="0.001" min="0"
