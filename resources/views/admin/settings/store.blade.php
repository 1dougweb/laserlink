...
@section('title', 'Loja Virtual - Laser Link')
@section('page-title', 'Configurações da Loja Virtual')

@section('content')
<div class="space-y-6" x-data="storeSettings()">
    <form method="POST" action="{{ route('admin.store-settings.update') }}" x-init="init()" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" 
                               ...
                               value="1"
                               {{ ($settings['store_bottom_menu_enabled'] ?? true) ? 'checked' : '' }}
                               class="rounded border-gray-300 text-primary shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50">
                ...
                <div>
                    ...
                    <select name="store_menu_style" 
                        ...
                        <option value="modern" {{ ($settings['store_menu_style'] ?? 'modern') === 'modern' ? 'selected' : '' }}>Moderno</option>
                        <option value="classic" {{ ($settings['store_menu_style'] ?? 'modern') === 'classic' ? 'selected' : '' }}>Clássico</option>
                        <option value="minimal" {{ ($settings['store_menu_style'] ?? 'modern') === 'minimal' ? 'selected' : '' }}>Minimalista</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="store_menu_position" 
                            ...
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                        <option value="bottom" {{ ($settings['store_menu_position'] ?? 'bottom') === 'bottom' ? 'selected' : '' }}>Inferior</option>
                        <option value="top" {{ ($settings['store_menu_position'] ?? 'bottom') === 'top' ? 'selected' : '' }}>Superior</option>
                    </select>
                </div>
            </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea 
                        ...
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent font-mono text-sm">{{ old('contact_map_embed_url', $settings['contact_map_embed_url'] ?? '') }}</textarea>
    ...
    <x-file-manager-modal 
        modal-id="categoryImageManagerModal" 
        title="Selecionar Imagem da Categoria" 
        on-select-callback="selectCategoryImage" />
    
    <!-- Componente File Manager para Banners Desktop -->
    <x-file-manager-modal 
        modal-id="bannerDesktopFileManager" 
        title="Selecionar Banner Desktop" 
        on-select-callback="selectBannerDesktop" />
    
    <!-- Componente File Manager para Banners Mobile -->
    <x-file-manager-modal 
        modal-id="bannerMobileFileManager" 
        title="Selecionar Banner Mobile" 
        on-select-callback="selectBannerMobile" />
...
function bannerManager() {
    return {
        uploadedBanners: @json($bannersForAlpine ?? []),
...
function bannerMobileManager() {
    return {
        uploadedBannersMobile: @json($bannersMobileForAlpine ?? []),
...
function homeCategories() {
    return {
        categories: @json(json_decode($settings['home_categories'] ?? '[]', true)),
...
function faqManager() {
    return {
        faqs: @json($settings['contact_faq'] ? json_decode($settings['contact_faq'], true) : []),
        sortable: null,
