...
@section('title', 'Editar Produto - Laser Link')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="short_description" 
                              ...
                              rows="2"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('short_description') border-red-500 @enderror">{{ old('short_description', $product->short_description) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" 
                              name="description" 
                              rows="10"
                              class="quill-editor">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" 
                               id="price" 
                               name="price" 
                               value="{{ old('price') ? old('price') : ($product->price ? number_format((float)$product->price, 2, ',', '.') : '') }}"
                               placeholder="0,00"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" 
                               id="sale_price" 
                               name="sale_price" 
                               value="{{ old('sale_price') ? old('sale_price') : ($product->sale_price ? number_format((float)$product->sale_price, 2, ',', '.') : '') }}"
                               placeholder="0,00"
                    ...
                    @enderror
                            ...
                            <div>
                                ...
                                <textarea id="meta_description" 
                                          ...
                                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('meta_description') border-red-500 @enderror">{{ old('meta_description', $product->meta_description) }}</textarea>
                                <div class="flex justify-between text-xs mt-1">
                                    <span class="text-gray-500">Usado no Google</span>
                        ...
                        <div x-show="showSeoPreview" 
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="text-green-700 text-sm">
                                    {{ url('/') }}/produto/<span x-text="seoData.slug || '{{ $product->slug ?? 'nome-do-produto' }}'"></span>
                                </div>
                                
                                <!-- Title -->
                                <div class="text-blue-600 text-lg font-medium leading-tight">
                                    <span x-text="seoData.meta_title || '{{ $product->meta_title ?: $product->name ?: 'Título do produto aparecerá aqui' }}'"></span>
                                </div>
                                
                                <!-- Description -->
                                <div class="text-gray-600 text-sm leading-relaxed">
                                    <span x-text="seoData.meta_description || '{{ $product->meta_description ?: $product->short_description ?: 'Descrição do produto aparecerá aqui' }}'"></span>
                                </div>
                                ...
                                <div class="mt-2 p-2 bg-gray-50 rounded">
                                    ...
                                    <div class="flex flex-wrap gap-1.5">
                                        <template x-if="seoData.meta_keywords">
                                            <template x-for="keyword in (seoData.meta_keywords || '{{ $product->meta_keywords }}').split(',').map(k => k.trim()).filter(k => k)" :key="keyword">
                                                <span class="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs font-medium" x-text="keyword"></span>
                                            </template>
                                        </template>
                                        <template x-if="!seoData.meta_keywords && !'{{ $product->meta_keywords }}'">
        ...
        <div class="mt-3">
            ...
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-medium text-gray-900">
                    Gerenciar Campos Extras - {{ $product->name }}
                </h3>
...
window.existingCustomAttributes = {!! json_encode($product->custom_attributes ?? []) !!};
...
window.productFeaturedImage = {!! json_encode($product->featured_image ? url('images/' . $product->featured_image) : null) !!};
window.productFeaturedImagePath = {!! json_encode($product->featured_image ?? '') !!};
window.productGalleryImages = {!! json_encode(
...
window.extraFieldsData = function() {
    return {
        ...
        loadProductFieldOptions(fieldId) {
            const url = '/admin/products/{{ $product->id }}/extra-fields/' + fieldId + '/options';
        ...
        saveFieldOptions() {
            ...
            const url = '/admin/products/{{ $product->id }}/extra-fields/' + this.selectedField.id + '/settings';
...
function loadExtraFieldsContent() {
    ...
    fetch(`{{ route('admin.products.extra-fields', $product) }}`, {
        method: 'GET',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json',
...
async function updateFieldOrder() {
    ...
    try {
        const response = await fetch(`{{ route('admin.products.extra-fields.reorder', $product) }}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
...
async function removeFieldFromProduct(fieldId, fieldName) {
    if (confirm(`Tem certeza que deseja remover o campo "${fieldName}"?`)) {
        ...
        const url = `/admin/products/{{ $product->id }}/extra-fields/${fieldId}`;
