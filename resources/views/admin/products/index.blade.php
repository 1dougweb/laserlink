...
<div class="p-6">
    ...
    <!-- Filtros de Pesquisa -->
    <div class="bg-white rounded-lg shadow mb-6" x-data="{ showFilters: {{ request()->hasAny(['search', 'category_id', 'status', 'featured', 'price_min', 'price_max']) ? 'true' : 'false' }} }">
        <div class="p-4 border-b border-gray-200">
            <button @click="showFilters = !showFilters" 
                ...
                <div class="flex items-center space-x-2">
                    ...
                    @if(request()->hasAny(['search', 'category_id', 'status', 'featured', 'price_min', 'price_max']))
        ...
        <form method="GET" action="{{ route('admin.products') }}" x-show="showFilters" x-cloak>
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" 
                               ...
                               value="{{ request('price_min') ? number_format((float)request('price_min'), 2, ',', '.') : '' }}"
                               placeholder="0,00"
                               class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                        <input type="hidden" id="price_min_value" name="price_min" value="{{ request('price_min') }}">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" 
                               ...
                               value="{{ request('price_max') ? number_format((float)request('price_max'), 2, ',', '.') : '' }}"
                               placeholder="0,00"
                               class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                        <input type="hidden" id="price_max_value" name="price_max" value="{{ request('price_max') }}">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select id="featured" 
                            ...
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">
                        <option value="">Todos</option>
                        <option value="1" {{ request('featured') === '1' ? 'selected' : '' }}>Sim</option>
                        <option value="0" {{ request('featured') === '0' ? 'selected' : '' }}>Não</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="sort_by" 
                        ...
                        <option value="stock_quantity" {{ request('sort_by') === 'stock_quantity' ? 'selected' : '' }}>Estoque</option>
                    </select>
                </div>
            </div>
                    ...
                    @foreach($products as $product)
                        <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <div class="flex items-center min-w-[250px]">
                                    ...
                                    @else
                                        <img class="h-10 w-10 rounded-lg object-contain mr-4 flex-shrink-0 bg-gray-100 p-1" 
                                             src="{{ url('images/general/callback-image.svg') }}" 
                                             alt="{{ $product->name }}">
                                    @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm text-gray-900">{{ $product->category->name ?? 'Sem categoria' }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="flex items-center space-x-2 justify-end">
                                    ...
                                    <form method="POST" action="{{ route('admin.products.toggle-status', $product) }}" class="inline">
                                        ...
                                        <button type="submit" 
                                            ...
                                            <i class="bi {{ $product->is_active ? 'bi-toggle-on' : 'bi-toggle-off' }} text-lg"></i>
                                        </button>
                                    </form>
