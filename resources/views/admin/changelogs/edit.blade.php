...
@section('title', 'Editar Atualização - Laser Link')
@section('page-title', 'Editar Atualização')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" 
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('description') border-red-500 @enderror">{{ old('description', $changelog->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
function changelogForm() {
    return {
        features: @json(old('features', $changelog->features ?? [])),
        improvements: @json(old('improvements', $changelog->improvements ?? [])),
        fixes: @json(old('fixes', $changelog->fixes ?? [])),
