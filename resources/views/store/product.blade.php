...
@section('title', $product->name . ' - Laser Link')
...
{!! json_encode($schemaProduct, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) !!}
</script>
    ...
    <nav class="bg-gray-50 py-4">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <ol class="flex items-center space-x-2 text-sm text-gray-500">
                <li><a href="{{ route('store.index') }}" class="hover:text-primary transition-colors">Início</a></li>
                ...
                <div class="flex flex-col lg:flex-row gap-4">
                    ...
                    <div class="hidden lg:block relative flex flex-col w-24 flex-shrink-0">
                        ...
                        <div x-ref="thumbnailsContainer" 
                             class="flex flex-col gap-3 overflow-y-scroll scroll-smooth {{ count($allImages) > 5 ? 'max-h-[470px]' : '' }}"
                             @scroll="updateThumbnailScrollPosition()"
                             style="-ms-overflow-style: none; scrollbar-width: none; {{ count($allImages) > 5 ? '' : 'max-height: fit-content;' }}">
                            @foreach($allImages as $index => $image)
                                <div class="aspect-square bg-gray-100 rounded-lg overflow-hidden cursor-pointer border transition-all duration-200 shadow-sm hover:shadow-md flex-shrink-0 w-24"
                                     :class="currentImageIndex === {{ $index }} ? 'border-gray-400' : 'border-gray-200 hover:border-gray-400'"
                                     @click="changeImage({{ $index }})">
                                    <img src="{{ url('images/' . $image) }}" 
                                         alt="{{ $product->name }} - {{ $product->category->name }} - Imagem {{ $index + 1 }}" 
                                         class="w-full h-full object-cover"
                                         onerror="this.src='{{ url('images/general/callback-image.svg') }}'; this.classList.remove('object-cover'); this.classList.add('object-contain');">
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="flex-1 w-full aspect-square lg:w-[470px] lg:h-[470px] bg-white rounded-2xl overflow-hidden shadow-lg relative">
                        ...
                        <div class="w-full h-full relative overflow-hidden">
                            @foreach($allImages as $index => $image)
                                <div x-show="currentImageIndex === {{ $index }}"
                                     ...
                                     @click="openImageModal('{{ url('images/' . $image) }}')"
                                     style="display: none;">
                                    <img src="{{ url('images/' . $image) }}" 
                                         alt="{{ $product->name }} - {{ $product->category->name }} - Foto detalhada {{ $index + 1 }} - Laser Link" 
                        ...
                        <div class="absolute bottom-4 right-4 bg-black/60 text-white px-3 py-1.5 rounded-full text-sm font-medium backdrop-blur-sm">
                            <span x-text="currentImageIndex + 1"></span> / {{ count($allImages) }}
                        </div>
                        @endif
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            @if($product->sku)
                                <span class="text-sm text-gray-500">SKU: <span class="font-mono">{{ $product->sku }}</span></span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6 shadow-md">
                        @if($product->sale_price && $product->sale_price > 0)
                            <div class="space-y-2">
                                ...
                                <span class="text-4xl font-bold text-green-600">R$ {{ number_format($product->sale_price, 2, ',', '.') }}</span>
                                ...
                                <div class="inline-flex items-center bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold">
                                    ...
                                    Economia de {{ $product->discount_percentage }}%
                                </div>
                            </div>
                        @elseif($product->auto_calculate_price)
                            <div class="space-y-2">
                                <div class="flex items-baseline gap-3">
                                    ...
                                    @if(count($optionsToShow) > 0)
                                        <div class="extra-field-container">
                                            ...
                                            @if($field->type === 'select')
                                                <!-- Dropdown Select -->
                                                <select name="extra_fields[{{ $field->slug }}]" 
                                                        x-model="selectedOptions['{{ $field->slug }}']"
                                                    ...
                                                    @foreach($optionsToShow as $option)
                                                        <option value="{{ $option['value'] }}" 
                                                                ...
                                                                data-price-type="{{ $option['price_type'] ?? 'fixed' }}">
                                                            {{ $option['label'] }}
                                                            @if(isset($option['price']) && $option['price'] > 0)
                                                                @if(isset($option['price_type']) && $option['price_type'] === 'percentage')
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    @foreach($optionsToShow as $option)
                                                        <label class="relative flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all duration-200 group"
                                                               :class="selectedOptions['{{ $field->slug }}'] === '{{ $option['value'] }}' 
                                                                   ? 'border-primary bg-primary/5 shadow-md' 
                                                                   : 'border-gray-200 hover:border-gray-300 hover:shadow-sm bg-white'">
                                                            <input type="radio" 
                                                                   name="extra_fields[{{ $field->slug }}]" 
                                                                   value="{{ $option['value'] }}"
                                                                   x-model="selectedOptions['{{ $field->slug }}']"
                                                                   @change="calculateTotalPrice()"
                                                                   class="sr-only">
                                                            ...
                                                            <div class="flex-shrink-0">
                                                                <div class="w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all"
                                                                    ...
                                                                    <i x-show="selectedOptions['{{ $field->slug }}'] === '{{ $option['value'] }}'" 
                                                ...
                                                <div class="space-y-2">
                                                    @foreach($optionsToShow as $option)
                                                        <label class="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors"
                                                               :class="selectedOptions['{{ $field->slug }}'] && selectedOptions['{{ $field->slug }}'].includes('{{ $option['value'] }}') ? 'border-primary bg-primary/5' : ''">
                                                            <input type="checkbox" 
                                                                   value="{{ $option['value'] }}"
                                                                   x-model="selectedOptions['{{ $field->slug }}']"
                                                                   @change="calculateTotalPrice()"
                                                            ...
                                                            <div class="flex-shrink-0">
                                                                <div class="w-6 h-6 rounded border-2 flex items-center justify-center transition-all"
                                                                     :class="selectedOptions['{{ $field->slug }}'] && selectedOptions['{{ $field->slug }}'].includes('{{ $option['value'] }}') 
                                                                         ? 'border-green-500 bg-green-500' 
                                                                         : 'border-gray-300'">
                                                                    <i x-show="selectedOptions['{{ $field->slug }}'] && selectedOptions['{{ $field->slug }}'].includes('{{ $option['value'] }}')" 
                                                                       class="bi bi-check-lg text-white font-bold"></i>
                                                                </div>
                                                            </div>
                                            ...
                                            @elseif($field->type === 'image' || $field->type === 'color')
                                                    ...
                                                    @foreach($optionsToShow as $option)
                                                        <div class="relative">
                                                            <input type="radio"
                                                                   name="extra_fields[{{ $field->slug }}]"
                                                                   value="{{ $option['value'] }}"
                                                                   id="option_{{ $field->slug }}_{{ $loop->index }}"
                                                                   x-model="selectedOptions['{{ $field->slug }}']"
                                                                   @change="calculateTotalPrice()"
                                                                   class="sr-only">
                                                            
                                                            <label for="option_{{ $field->slug }}_{{ $loop->index }}"
                                                                ...
                                                                <div class="relative w-full aspect-square rounded-lg overflow-hidden border-2 transition-all duration-200"
                                                                    ...
                                                                    @elseif(isset($option['color_hex']) && $option['color_hex'])
                                                                        <!-- Cor Sólida -->
                                                                    ...
                                                                    <div x-show="selectedOptions['{{ $field->slug }}'] === '{{ $option['value'] }}'"
                                                ...
                                                <div class="flex flex-wrap gap-2">
                                                    @foreach($optionsToShow as $option)
                                                        <span class="inline-flex items-center px-3 py-1.5 rounded-md text-sm border border-gray-300 text-gray-700 bg-white">
                                                            ...
                                                            @if(isset($option['price']) && $option['price'] > 0)
                                                                @if(isset($option['price_type']) && $option['price_type'] === 'percentage')
                                                                    <span class="ml-1.5 font-semibold text-primary">+{{ number_format($option['price'], 0) }}%</span>
                                                                @else
                                                                    <span class="ml-1.5 font-semibold text-primary">+R$ {{ number_format($option['price'], 2, ',', '.') }}</span>
                                                                @endif
                                                            @endif
                                                        </span>
                                @endforeach
            ...
            <div x-data="{ activeTab: 'description' }">
                ...
                <div class="bg-white rounded-b-2xl shadow-lg p-8">
                    ...
                    <div x-show="activeTab === 'reviews'" x-transition:enter="transition ease-out duration-300"
                         ...
                         x-transition:enter-end="opacity-100 transform translate-y-0"
                         style="display: none;">
                        @include('store.partials.product-reviews', ['product' => $product, 'reviewsStats' => $reviewsStats])
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="relative group w-full sm:max-w-[300px] sm:mx-auto flex flex-col bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow"
                            ...
                            if (!this.inCart) {
                                ...
                                if (existingItem) {
                                    ...
                                    cart.push({
                                        ...
                                        image: this.relProduct.featured_image || '{{ url('images/general/callback-image.svg') }}',
                                        quantity: 1
                                    });
                                }
                    ...
                    <div class="relative w-full aspect-square mb-4 flex-shrink-0 overflow-hidden">
                        @if($relatedProduct->featured_image)
                            <img src="{{ url('images/' . $relatedProduct->featured_image) }}" 
                                 alt="{{ $relatedProduct->name }} - {{ $relatedProduct->category->name }} - Laser Link" 
                        ...
                        @else
                            <img src="{{ url('images/general/callback-image.svg') }}" 
                                 alt="{{ $relatedProduct->name }} - Produto relacionado Laser Link"
                        ...
                        <div class="absolute left-3 top-3 flex flex-col gap-2 z-10">
                            ...
                            @if($relatedProduct->is_on_sale && $relatedProduct->price)
                    ...
                    <div class="flex flex-col flex-grow space-y-2 p-4">
                        <!-- Title -->
                            <a href="{{ route('store.product', $relatedProduct->slug) }}" 
                        ...
                        <div class="flex items-center justify-between gap-2 min-h-[2rem]">
                            <div class="flex items-center gap-2">
                                <p class="text-lg font-semibold text-[#272343]">
                                    R$ {{ number_format($relatedProduct->final_price, 2, ',', '.') }}
                                </p>
                                @if($relatedProduct->is_on_sale && $relatedProduct->price)
                            ...
                            <x-product-rating 
                                ...
                                :count="$relatedProduct->rating_count ?? 0"
                                :showCount="false"
                                size="md"
                            />
                        </div>
...
function productPage() {
    return {
        ...
        selectedOptions: {},
        quantity: 1,
        basePrice: {{ $product->auto_calculate_price ? ($product->min_price ?? 0) : ($product->final_price ?? 0) }},
        extraCost: 0,
        ...
        init() {
            ...
            @if($extraFields && $extraFields->count() > 0)
                @foreach($extraFields as $field)
                    @if($field->type === 'checkbox')
                        this.customization['{{ $field->slug }}'] = [];
                        this.selectedOptions['{{ $field->slug }}'] = [];
                    @else
                        this.customization['{{ $field->slug }}'] = '';
                        this.selectedOptions['{{ $field->slug }}'] = '';
                    @endif
            ...
            const cart = JSON.parse(localStorage.getItem('cart') || '[]');
            this.inCart = cart.some(item => item.product_id === {{ $product->id }});
            ...
            window.addEventListener('favoritesUpdated', () => {
                this.syncFavorites();
                this.isFavorite = this.favorites.includes({{ $product->id }});
            });
        ...
        toggleFavorite(productId) {
            ...
            if (productId === {{ $product->id }}) {
                const productData = {
                    ...
                    slug: '{{ $product->slug }}',
                    price: {{ $product->final_price }},
                    image_url: '{{ $product->featured_image ? url('images/' . $product->featured_image) : url('images/general/callback-image.svg') }}'
                };
            ...
            @if($extraFields && $extraFields->count() > 0)
                @foreach($extraFields as $field)
                    if ('{{ $field->slug }}' === fieldSlug) {
                        ...
                        const options_{{ $field->id }} = @json($optionsToShow);
                        const found = options_{{ $field->id }}.find(opt => opt.value === value);
                        return found;
                    }
                @endforeach
            @endif
        ...
        async submitCustomization() {
            ...
            @foreach($extraFields as $field)
                @if($field->pivot->is_required)
                    if (!this.customization['{{ $field->slug }}'] || this.customization['{{ $field->slug }}'].length === 0) {
            ...
            try {
                ...
                cart.push({
                    ...
                    name: cartData.product_name || '{{ $product->name }}',
                    slug: cartData.product_slug || '{{ $product->slug }}',
                    price: cartData.unit_price,
                    image: cartData.product_image || '{{ $product->featured_image ? url('images/' . $product->featured_image) : url('images/general/callback-image.svg') }}',
        ...
        validateRequiredFields() {
            @if($extraFields && $extraFields->count() > 0)
                @foreach($extraFields as $field)
                    @if($field->pivot->is_required)
                        if (!this.selectedOptions['{{ $field->slug }}'] || 
                            (Array.isArray(this.selectedOptions['{{ $field->slug }}']) && this.selectedOptions['{{ $field->slug }}'].length === 0)) {
                            this.showErrorMessage('Por favor, selecione uma opção para: {{ $field->name }}');
                            return false;
                        }
                    @endif
                @endforeach
        ...
        registerProductView() {
            ...
            const productData = {
                ...
                slug: '{{ $product->slug }}',
                price: {{ $product->final_price }},
                image: '{{ $product->featured_image ? url('images/' . $product->featured_image) : url('images/general/callback-image.svg') }}'
            };
